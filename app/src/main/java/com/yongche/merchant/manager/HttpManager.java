package com.yongche.merchant.manager;


import android.text.TextUtils;

import com.yongche.merchant.app.MyApplication;
import com.yongche.merchant.interfaces.ProjectAPI;
import com.yongche.merchant.utils.RetrofitUtil;
...
public class HttpManager {
    ...
    public void postMethod(String url, Observer<String> observer, Map map, String[] carjb, String[] card, String[] c, String[] d, String[] e, String[] f, String[] g, String[] h, String[] i, String[] j) {
        if (MyApplication.getToken() != null)
            map.put("accessToken", MyApplication.getToken());
        if (MyApplication.getUserId() != null)
            map.put("userId", MyApplication.getUserId());
        ...
        map.put("os", "android");
        Observable<String> observable = RetrofitUtil.getInstance().get(ProjectAPI.class).postMethod(url, map, carjb, card, c, d, e, f, g, h, i, j);
     ...
     * @param observer
     */
    public void postTochMethod(String baseUrl, String url, Observer<String> observer, boolean isTouch, String userId) {

        //获取被观察者
        Observable<String> observable = RetrofitUtil.getInstance().get(ProjectAPI.class).postTouchMethod(url, isTouch, userId);
        //在子线程中执行请求，在主线程观察，将信息设置给观察者
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(observer);
